...
export default createPaginationContainer(OrganizationComputeCredits,
  {
    info: graphql`
      ...
      @argumentDefinitions(
        ...
        transactions(
          last: $count
          after: $cursor
        ) @connection(key: "OrganizationComputeCredits_transactions") {
          edges {
            node {
  ...
  },
  {
    ...
    query: graphql`
      query OrganizationComputeCreditsQuery(
        $count: Int!
        $cursor: String
        $organization: String!
      ) {
