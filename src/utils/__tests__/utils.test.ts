import { useBuildStatusColor, useFaviconColor, useTaskStatusColor } from '../colors';
...
describe('utils/colors tests', () => {
  ...
  it('gets the correct build status color', () => {
    expect(useBuildStatusColor('ABORTED')).toBe(cirrusColors.lightFailure);
  });

  it('gets the correct task status color', () => {
    expect(useTaskStatusColor('FAILED')).toBe(cirrusColors.failure);
  });

  it('gets the correct favicon color', () => {
    expect(useFaviconColor('EXECUTING')).toBe(cirrusColors.executing);
  });
});
